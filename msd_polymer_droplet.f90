...
!DIFFUSION OF MONOMERS IN DROPLET
!*************************************************************************************!
      ...
      Use Var_Control
      implicit none
      logical ::status_go, status_jump
      real(kind=double)::dx,dy,dz,sq_disp, sq_disp_droplet_com
      real(kind=double) ::dx_droplet, dy_droplet, dz_droplet
      real(kind=double), allocatable,dimension(:)::msd, msd_dc
      integer::corr_frame
      integer::iframe,jframe,kframe,ip, icount
      integer :: idd1,idd2
     
      integer:: count_norm_step,count_norm_polymer,count_norm, count_norm_dc



      corr_frame = int((Nstep-Nstepskip)/10)
      allocate(msd(corr_frame), msd_dc(corr_frame))!,msd_free(corr_frame))
      ...
      do iframe = 1, corr_frame
         ...
         do jframe=1, (Nstep-Nstepskip)
            ...
            if ((jframe+iframe) .gt. (Nstep-Nstepskip)) exit
               ...
               do ip = 1, Npolymer
                  ...
                  if (id_droplet(ip,jframe) .eq. 1 .and. id_droplet(ip,(jframe+iframe)) .eq. 1) then
                     ...
                     if (status_go .eqv. .FALSE.) then 
                       ...
                       idd1= id_particle_in_cluster(ip, jframe)
                       idd2 = id_particle_in_cluster(ip, jframe+iframe)

                       !dx_droplet=com_droplet_x(idd2,(jframe+iframe))-com_droplet_x(idd1,(jframe))
                       !dy_droplet=com_droplet_y(idd2,(jframe+iframe))-com_droplet_y(idd1,(jframe))
                       !dz_droplet=com_droplet_z(idd2,(jframe+iframe))-com_droplet_z(idd1,(jframe))


                       
                       
                       !count_norm = count_norm+1
                       dx= com_x(ip,(jframe+iframe))-com_x(ip,(jframe))
                       dy= com_y(ip,(jframe+iframe))-com_y(ip,(jframe))
                       dz=com_z(ip,(jframe+iframe))-com_z(ip,(jframe))
               
                       !dx = dx-dx_droplet
                       !dy = dy-dy_droplet
                       !dz=dz-dz_droplet
                       sq_disp = (dx**2+dy**2+dz**2)
                       !sq_disp_droplet_com = (dx_droplet**2 +dy_droplet**2 +dz_droplet**2)
                       msd(iframe)= msd(iframe)+sq_disp
                       !msd_dc(iframe) = msd_dc(iframe) +sq_disp_droplet_com
                     end if
                  end if
               end do 
               !msd_droplet(iframe)=msd_droplet(iframe)/count_norm_polymer
               
         end do
         msd(iframe)=msd(iframe)/count_norm
         write(5000,*) (iframe), msd(iframe)
      end do
      ...
      do iframe = 1, corr_frame
       ...
       do jframe = 1, (Nstep-Nstepskip)
          if((jframe+iframe) .gt. (Nstep-Nstepskip)) exit
          do icount =  1, clustersum
             if(forwarded_array_cluster_size(icount,jframe) .eq. largest_cluster(jframe)) then
              if ( forwarded_array_cluster_size(icount,jframe+iframe) .eq. largest_cluster(jframe+iframe)) then
                 status_jump= .TRUE.
                 do kframe = jframe+1, (jframe+iframe-1)
                     
                    if(forwarded_array_cluster_size(icount,kframe) .ne. largest_cluster(kframe)) then
                      status_jump=.FALSE.
                      exit
                    end if
                 end do
                 if (status_jump .eqv. .FALSE.) then
                        cycle
                 else    
                    dx_droplet =com_droplet_x(icount,(jframe+iframe))-com_droplet_x(icount,(jframe)) 
                    dy_droplet=com_droplet_y(icount,(jframe+iframe))-com_droplet_y(icount,(jframe))
                    dz_droplet=com_droplet_z(icount,(jframe+iframe))-com_droplet_z(icount,(jframe))
                    sq_disp_droplet_com = (dx_droplet**2 +dy_droplet**2 +dz_droplet**2)
                    msd_dc(iframe) = msd_dc(iframe) +sq_disp_droplet_com
                    count_norm_dc= count_norm_dc +1
                 end if
              end if
             end if 
          end do
       end do
       write(7000, *) iframe, msd_dc(iframe)/count_norm_dc
      end do
      end subroutine GETDIFFUSION
