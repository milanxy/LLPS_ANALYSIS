...
!*************************************************************************************! 
      ...
      real(kind=double)::boxlen, binsize, reduced_T
...
!*************************************************************************************! 
      ...
      !end do
      !call GETDIFFUSION
     write(5000,*) energy_electrostatic_per_polymer/(Nstep-Nstepskip),  &
           sqrt((sq_elec/(Nstep-Nstepskip))-(energy_electrostatic_per_polymer/(Nstep-Nstepskip))**2) 
     write(6000,*) energy_hydrophobic_per_polymer/(Nstep-Nstepskip),     &
sqrt((sq_hyd/(Nstep-Nstepskip))-(energy_hydrophobic_per_polymer/(Nstep-Nstepskip))**2)
      end program main
...
!*************************************************************************************!
      ...
      real(kind=double)::qiresi,qjresi
      real(kind=double)::Ecol,eps_solvent=80.0,salt_conc=0.02,kappa,bk,tot_E_col, tot_E_hyd, Ehyd
      real(kind=double)::r12,r10,epsil=0.2, sigma=7.0
      integer :: count_free_pts, test_var
      real(kind=double) ::av_cn_free
      logical:: state_poly
      !kappa=0.32*sqrt(salt_conc)
      kappa=SQRT((2529.118* salt_conc*1.0)/(eps_solvent*295*reduced_T))
      bk=exp(kappa*1.4)/(1+(kappa*1.4))
      !do ip = 1, Npolymer
      !   total_E_col(ip)=0
      !end do
      tot_E_col =0
               ...
               if (id_droplet(jp,istep).eq. 1 ) then
                   ...
                   do iresi = ll+1,ll+Nbead_per_resi
                    ...
                    do jresi = mm+1, mm+Nbead_per_resi
                         ...
                         !if (dijin .lt. 20) then
                          if (seq_char(iresi) .eq. 'K' .and. seq_char(jresi).eq.'E') then
                             qiresi=1
                             qjresi=-1
                             Ecol= 332*(qiresi*qjresi)*exp(-kappa*dijin)/(dijin*eps_solvent)
                             tot_E_col=tot_E_col+Ecol
                          else if (seq_char(iresi) .eq. 'E' .and. seq_char(jresi).eq.'K') then
                             qiresi=-1
                             qjresi=1
                             Ecol=332*(qiresi*qjresi)*exp(-kappa*dijin)/(dijin*eps_solvent)
                             tot_E_col=tot_E_col+Ecol
                          else if (seq_char(iresi) .eq. 'K' .and. seq_char(jresi).eq.'K') then
                             qiresi=1
                             qjresi=1
                             Ecol=332*(qiresi*qjresi)*exp(-kappa*dijin)/(dijin*eps_solvent)
                             tot_E_col=tot_E_col+Ecol
                          else if (seq_char(iresi) .eq. 'E' .and. seq_char(jresi).eq.'E') then
                             qiresi=-1
                             qjresi=-1
                             Ecol=332*(qiresi*qjresi)*exp(-kappa*dijin)/(dijin*eps_solvent)
                             tot_E_col=tot_E_col+Ecol
                          else if (seq_char(iresi) .eq. 'F' .and. seq_char(jresi).eq.'F') then
                             r12 =5*(sigma/dijin)**12
                             r10 =6*(sigma/dijin)**10
                             Ehyd =epsil*(r12-r10)
                             tot_E_hyd = tot_E_hyd+Ehyd
                         end if
                        !end if
                         !total_E_col(ip) = total_E_col(ip)+Ecol
                         !total_E_col(jp) = total_E_col(jp)+Ecol
                    end do
                   end do
               end if
            end do
      ...
      tot_E_col=tot_E_col*bk      
      print*,istep, largest_size_of_cluster, (tot_E_hyd/largest_size_of_cluster)
      energy_electrostatic_per_polymer=energy_electrostatic_per_polymer+(tot_E_col/largest_size_of_cluster) 
      sq_elec = sq_elec + (tot_E_col/largest_size_of_cluster)**2
      energy_hydrophobic_per_polymer=energy_hydrophobic_per_polymer+(tot_E_hyd/largest_size_of_cluster)
      sq_hyd = sq_hyd +(tot_E_hyd/largest_size_of_cluster)**2
